...
public class ConsoleGame {
	...
	public void playGame() {
		...
		while(!level.isFinished()){
			...
			int num = 999;
			String mess = "How many spaces would you like to move this vehicle?";
			System.out.println(level);//use this line to print out the current level with the header row information
			System.out.println("What's your next move? (Moves so far: " + level.getNumOfMoves() + ")");
			Location move = getLocationFromUser(NUM_ROWS, NUM_COLS);
			if(level.getVehicleAt(move) == null) {
				System.out.println("The location selected doesnt have a car, try again!");
				continue;
			}
			while(!movePossible) {
				num = getInteger(mess);
				movePossible = level.canMoveVehicleAt(move, num);
				mess = "Sorry but that vehicle can't be moved to where you want, please try agian \n"
						+ "How many spaces would you like to move this vehicle? \n"
						+ "Enter 0 to select a different car";
			}
			if(num != 0) {
				level.moveVehicleAt(move, num);
				level.incNumOfMoves();
			}
			
		}
		
		System.out.println("Congratulation Brother, you did it \nSee you soon, Bye Bye");
	}
